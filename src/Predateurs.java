...
public class Predateurs extends Boids{
     ...
     public Predateurs(Point[] positions, Point[] vitesses, long pas, float maxspeed, float maxforce, int r){
       super(positions, vitesses, pas, maxspeed, maxforce, r);
     }

     public void chase(Boids proies){
       float distance = 200;
       int ligne = this.getPositions().length;
       int colonne = proies.getPositions().length;
       ...
       for (int j =0; j<ligne; j++){
         ...
         for (int i= 0; i<colonne; i++) {
           ...
           float dx = -this.getPositions()[j].x + proies.getPositions()[i].x;
           float dy = -this.getPositions()[j].y + proies.getPositions()[i].y;
     ...
     public void move(int width, int heigth, Proies proies){
       this.align();
       this.chase(proies);
       this.separate();
       this.cohesion();
